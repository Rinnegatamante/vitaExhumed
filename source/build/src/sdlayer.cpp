...
char cur_dir[MAX_CURDIR_PATH] = "ux0:data/vitaExhumed/";
    ...
    else
    {
        ...
        for (int i = 0; i < numjoysticks; i++)
        {
            if ((joydev = SDL_JoystickOpen(i)))
            {
                ...
                joystick.numAxes = min((int32_t)9, (int32_t)SDL_JoystickNumAxes(joydev));
                joystick.numButtons = min((int32_t)32, (int32_t)SDL_JoystickNumButtons(joydev));
